# Supabase Setup & Hierarchical Schema

This file contains the SQL scripts to set up a multi-user, multi-garden system.
...
1.  **Profiles**: You can set up a Supabase Trigger to automatically create a entry in `profiles` when a user signs up.
2.  **User Plants**: This table replaces the `selectedPlants` array in the old JSON state. It allows for much more powerful querying (e.g., "how many users are growing basil?").
3.  **Hierarchy**: The flow is `Auth User` -> `Gardens` -> `User Plants` -> `Tracker Entries`.
